...
func logs(whenToEat: String) -> Meal? {
    if let meal = meals[whenToEat] {
        return meal
...
print(logs(whenToEat: "Breakfast"))
print(logs(whenToEat: "Dinner"))
...
func checkMealLog() -> [String: Any] {
    ...
    if let myCheck = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return myCheck
    } else {
        return [:]
    }
